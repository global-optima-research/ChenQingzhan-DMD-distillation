#!/bin/bash
# Minimal FFGO-to-FastGen feasibility run.
# Current default uses a backbone-compatible teacher placeholder so the
# training loop can be validated before real FFGO merged weights are ready.
...
RUN_NAME="${RUN_NAME:-ffgo_cd_32step_smoke_1000iter}"
MAX_ITER="${MAX_ITER:-1000}"
SAVE_ITER="${SAVE_ITER:-200}"
VAL_ITER="${VAL_ITER:-200}"
...
echo "[FFGO-CD] Steps: 32, Iter: ${MAX_ITER}"
...
torchrun --nproc_per_node=2 --standalone train.py \
    --config=fastgen/configs/experiments/WanT2V/our/exp08_ffgo_cd_32step_smoke.py \
    - trainer.ddp=False \
      trainer.fsdp=True \
