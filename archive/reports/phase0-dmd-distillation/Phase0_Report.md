...
> **Date:** 2026-03-23 (Updated from 2026-03-12)
> **Task:** Task 3 — DMD Distillation & Acceleration
> **Branch:** `Task3_dev_ChenHingChin`
> **Status:** Phase 0 + Phase 0.5 Complete
...
## 5.5 Phase 0.5: From-Scratch Training Experiments (2026-03-17 ~ 03-23)

Following the Phase 0 pretrained inference study, Phase 0.5 conducted **from-scratch training** of three distillation methods (ECT, CD, DMD2) using FastGen's native pipeline to evaluate actual training quality on video data.

### 5.5.1 ECT — 1000 iter, Single GPU

| Parameter | Value |
|-----------|-------|
| Config | Custom `config_cm_ct.py` (`use_cd=False`, no teacher) |
| GPU | 1× RTX 5090 32GB, 8-bit AdamW |
| Training | 1000 iter, batch=1, ~15s/iter |

**Result:** Completely blurry at both 1-step and 4-step inference. No recognizable content.

| ECT 1000 iter, 4-step |
|:---:|
| ![ECT 1000iter](results/frames/ect_1000iter.jpg) |
| *Prompt: "A cat walking on the grass in the sunshine" — completely blurry, unusable* |

### 5.5.2 CD — 1000 iter (Single GPU) & 5000 iter (2-GPU FSDP)

| Parameter | 1000 iter | 5000 iter |
|-----------|-----------|-----------|
| Config | `config_cm_cd.py` (`use_cd=True`, teacher ODE) | Same + `kimg_per_stage=5` |
| GPU | 1× RTX 5090, 8-bit AdamW | 2× RTX 5090, FSDP |
| Batch | 1 | 2 |

**Result (1000 iter, 4-step):** Rough outlines and motion visible, decent temporal consistency, but lacks sharpness.

**Result (5000 iter, 4-step):** Quality **DEGRADED** — more blurry than 2000 iter checkpoint. Root cause: curriculum 640× faster than original paper (`kimg=5` vs `3200`), batch 512× smaller (`2` vs `1024`).

| CD 1000 iter, 4-step |
|:---:|
| ![CD 1000iter](results/frames/cd_1000iter.jpg) |
| *Rough outlines visible but lacks sharpness — CD has some potential but fundamentally limited on video* |

### 5.5.3 DMD2 — 2000 iter, 4-GPU FSDP ✅

DMD2 is FastGen's **official and validated** video distillation method (VBench 83.24, 4-step).

**Memory optimization journey:**

| Attempt | Setup | VRAM/GPU | Result |
|---------|-------|----------|--------|
| 1 | Single GPU + 8-bit AdamW | 30.74 GB | ❌ OOM |
| 2 | 2-GPU FSDP + 8-bit AdamW | 30.41 GB | ❌ bitsandbytes incompatible with FSDP2 DTensor |
| 3 | 2-GPU FSDP + CPU Offload | 14.35 GB | ⚠️ Works but ~1200s/iter (data loading bottleneck) |
| **4** | **4-GPU FSDP + std AdamW** | **22-27 GB** | **✅ 16s/iter** |

**Training configuration:**

| Parameter | Value |
|-----------|-------|
| Config | FastGen native `config_dmd2.py` (VSD + GAN) |
| GPU | 4× RTX 5090 32GB (FSDP) |
| Optimizer | Standard AdamW (fp32), lr=1e-5 |
| Batch size | 4 (1/GPU, no gradient accumulation) |
| Iterations | 2000 |
| Training time | ~9 hours |
| Per-iter time | ~16s |
| Peak VRAM | 22-27 GB/GPU |

**Loss convergence:** Total loss decreased from 1.38 → ~1.10 within the first 200 iterations, then stabilized in the 1.04-1.17 range. GAN discriminator loss: 1.39 → 1.25. VSD loss fluctuated between 0.21-0.35.

### 5.5.4 DMD2 Inference Results — Visual Comparison

All videos generated with: **4-step student sampling, seed=42, guidance_scale=5.0**
Prompt: *"A cat walking on the grass in the sunshine"*

**Teacher baseline (50 steps, 170.88s) vs DMD2 Student (4 steps, ~6.7s) = 25.5× speedup**

| Model | Frame (t=40) |
|:------|:---:|
| **Teacher (50-step, 170.88s)** | ![Teacher](results/frames/teacher_50step.jpg) |
| **DMD2 400 iter (4-step, 6.73s)** | ![DMD2 400](results/frames/dmd2_400iter.jpg) |
| **DMD2 800 iter (4-step, 6.81s)** | ![DMD2 800](results/frames/dmd2_800iter.jpg) |
| **DMD2 1200 iter (4-step, 6.69s)** | ![DMD2 1200](results/frames/dmd2_1200iter.jpg) |
| **DMD2 1600 iter (4-step, 6.67s)** | ![DMD2 1600](results/frames/dmd2_1600iter.jpg) |
| **DMD2 2000 iter (4-step, 6.64s)** | ![DMD2 2000](results/frames/dmd2_2000iter.jpg) |

**Inference speed comparison:**

| Model | Steps | Time/Video | Speedup |
|-------|-------|-----------|---------|
| Teacher (Wan2.1-1.3B) | 50 | 170.88s | 1.0× |
| DMD2 Student (4-step) | 4 | ~6.7s | **25.5×** |
| CausVid (pretrained DMD) | 3 | 28.5s | 6.0× |
| rCM (pretrained) | 4 | 37.6s | 4.5× |

### 5.5.5 Analysis: Why 400 iter May Look Better Than 2000 iter

An unexpected finding: DMD2 at 400 iterations appears to produce higher visual quality than 2000 iterations. This is characteristic of **GAN training instability with small batch sizes**:

1. **Batch size gap:** Our `batch=4` vs FastGen official `batch=64` (16× smaller). The GAN discriminator receives extremely noisy gradient estimates, destabilizing the generator-discriminator balance.

2. **Discriminator overpowering:** As training progresses, the discriminator loss drops from 1.39 to 1.25 — the discriminator becomes too strong, forcing the generator to produce "safe" but detail-poor outputs to minimize adversarial loss.

3. **VSD loss instability:** `vsd_loss` fluctuates between 0.21-0.35 without monotonic decrease, reflecting score matching instability under small batch conditions.

4. **400 iter sweet spot:** At ~400 iter, the generator retains most pretrained quality while gaining early distillation speedup. Further training accumulates GAN instability, degrading quality.

**Recommended fixes:** Increase batch size via gradient accumulation (`batch_global=16+`), reduce GAN loss weight (0.03 → 0.01), or extend training to 6000 iter (FastGen official config) where the balance may restabilize.

### 5.5.6 Phase 0.5 Summary

| Method | Iter | GPUs | Steps | Time/Video | Quality | Status |
|--------|------|------|-------|-----------|---------|--------|
| ECT | 1000 | 1 | 4 | — | Completely blurry | ❌ |
| CD | 1000 | 1 | 4 | — | Outlines only | ⚠️ |
| CD | 5000 | 2 FSDP | 4 | — | Degraded (curriculum issue) | ❌ |
| **DMD2** | **2000** | **4 FSDP** | **4** | **6.7s** | **Converged, 25.5× speedup** | **✅** |

**Key conclusions:**

1. **ECT and CD are image-only methods**, not validated for video by FastGen. Our experiments confirm they cannot produce usable video output.

2. **DMD2 is the only viable video distillation method** on our hardware. It requires minimum 4× 32GB GPUs with FSDP (8-bit AdamW incompatible with FSDP2 DTensor).

3. **DMD2 achieves 25.5× inference speedup** (6.7s vs 170.88s), far exceeding the Phase 0 pretrained model results (CausVid 6.4×, rCM 4.9×).

4. **Quality may improve** with larger batch size, more iterations (6000 iter), or GAN loss weight tuning.
...
| DMD2 (2-GPU FSDP) | + FakeScore + Disc | **30.41 GB** | No (OOM) |
| **DMD2 (4-GPU FSDP)** | + FakeScore + Disc | **22-27 GB** | **Yes ✅** |
| f-distill | + FakeScore + Disc | **>31 GB** | Untested (similar to DMD2) |

**Root cause:** FSDP all-gathers full parameters during forward pass. Text encoder (UMT5-XXL ~10GB) replicated on each GPU.

**Phase 0.5 update:** DMD2 was successfully trained on 4-GPU FSDP (22-27 GB/GPU). Key finding: 8-bit AdamW (bitsandbytes) is incompatible with FSDP2 DTensor — standard fp32 AdamW must be used, requiring minimum 4 GPUs.
...
3. **VRAM is the primary constraint, but solvable:** DMD2 requires 4× 32GB GPUs with FSDP (22-27 GB/GPU). 8-bit AdamW is incompatible with FSDP2 DTensor. 2-GPU FSDP OOMs at 30.41 GB/GPU. CPU offload works but is impractically slow (~1200s/iter vs 16s/iter).

4. **ECT/CD are image-only methods — not viable for video:** ECT (1000 iter) and CD (1000-5000 iter) both failed to produce usable video output. FastGen intentionally excluded ECT/CD from WanT2V configs. Only distribution matching methods (DMD2, f-distill, LADD, CausVid) are validated for video.

5. **DMD2 from-scratch training works but is sensitive to batch size:** 2000 iter DMD2 on 4-GPU FSDP achieved stable convergence (loss 1.38→1.10) and 25.5× inference speedup. However, small batch size (4 vs official 64) causes GAN training instability — quality peaks around 400 iter then degrades.

6. **RTX 5090 compatibility gaps exist:** Flash attention libraries and custom CUDA extensions don't support Blackwell architecture (SM 12.0) yet. Pure PyTorch fallbacks work but may sacrifice performance.
...
|----------|--------|-----------|
| **Primary** | DMD2 (FastGen native) | Successfully trained in Phase 0.5; 25.5× speedup; official FastGen method (VBench 83.24) |
| **Secondary** | CausVid (DMD pretrained) | 6.4× speedup in Phase 0; mature pretrained checkpoint available |
| **Backup** | rCM (Consistency Model) | 4.9× speedup; NVlabs pretrained checkpoint for Wan2.1 |
| ~~Deprecated~~ | ~~ECT/CD~~ | ~~Not viable for video — image-only methods, confirmed in Phase 0.5~~ |

### 7.3 Hardware Recommendation for Phase 1

| Scenario | Model | Method | Minimum GPU | Recommended GPU |
|----------|-------|--------|-------------|-----------------|
| **DMD2 with 1.3B** | Wan2.1-1.3B | DMD2 | **4x RTX 5090 32GB** | 8x RTX 5090 32GB |
| DMD2 with 14B | Wan2.1-14B | DMD2 | 8x A100 80GB | 8x H100 80GB |
...
| **DMD2 (2000 iter, 4-GPU FSDP)** | Self-trained (Phase 0.5) | `/data/chenqingzhan/fastgen_output/fastgen/wan_dmd2/dmd2_4gpu_fsdp/checkpoints/` |
| DMD2 consolidated (400-2000 iter) | Consolidated from FSDP | `/data/chenqingzhan/fastgen_output/dmd2_consolidated/dmd2_{400..2000}iter.pth` |
