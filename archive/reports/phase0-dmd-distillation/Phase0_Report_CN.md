...
> **日期：** 2026-03-23（更新自 2026-03-12）
> **任务：** Task 3 — DMD 蒸馏与加速
> **分支：** `Task3_dev_ChenHingChin`
> **状态：** Phase 0 + Phase 0.5 已完成
...
## 5.5 Phase 0.5：从零训练实验（2026-03-17 ~ 03-23）

Phase 0 预训练推理研究之后，Phase 0.5 使用 FastGen 原生流水线对三种蒸馏方法（ECT、CD、DMD2）进行了**从零训练**，评估在视频数据上的实际训练质量。

### 5.5.1 ECT — 1000 iter，单卡

| 参数 | 值 |
|------|-----|
| 配置 | 自定义 `config_cm_ct.py`（`use_cd=False`，无 teacher） |
| GPU | 1× RTX 5090 32GB，8-bit AdamW |
| 训练 | 1000 iter，batch=1，~15s/iter |

**结果：** 1 步和 4 步推理均完全模糊，无法识别任何内容。

| ECT 1000 iter, 4 步推理 |
|:---:|
| ![ECT 1000iter](results/frames/ect_1000iter.jpg) |
| *提示词："A cat walking on the grass in the sunshine" — 完全模糊，不可用* |

### 5.5.2 CD — 1000 iter（单卡）& 5000 iter（双卡 FSDP）

| 参数 | 1000 iter | 5000 iter |
|------|-----------|-----------|
| 配置 | `config_cm_cd.py`（`use_cd=True`，teacher ODE） | 同上 + `kimg_per_stage=5` |
| GPU | 1× RTX 5090，8-bit AdamW | 2× RTX 5090，FSDP |
| 批大小 | 1 | 2 |

**结果（1000 iter, 4 步）：** 可见粗略轮廓和运动，时间一致性尚可，但缺乏清晰度。

**结果（5000 iter, 4 步）：** 质量**反而下降** — 比 2000 iter 检查点更模糊。根因：curriculum 比原论文快 640 倍（`kimg=5` vs `3200`），batch 小 512 倍（`2` vs `1024`）。

| CD 1000 iter, 4 步推理 |
|:---:|
| ![CD 1000iter](results/frames/cd_1000iter.jpg) |
| *可见粗略轮廓但缺乏清晰度 — CD 有一定潜力但在视频上根本受限* |

### 5.5.3 DMD2 — 2000 iter，4 卡 FSDP ✅

DMD2 是 FastGen **官方验证的**视频蒸馏方法（VBench 83.24，4 步）。

**显存优化历程：**

| 尝试 | 方案 | 显存/卡 | 结果 |
|------|------|---------|------|
| 1 | 单卡 + 8-bit AdamW | 30.74 GB | ❌ OOM |
| 2 | 双卡 FSDP + 8-bit AdamW | 30.41 GB | ❌ bitsandbytes 与 FSDP2 DTensor 不兼容 |
| 3 | 双卡 FSDP + CPU 卸载 | 14.35 GB | ⚠️ 可用但 ~1200s/iter（数据加载瓶颈） |
| **4** | **4 卡 FSDP + 标准 AdamW** | **22-27 GB** | **✅ 16s/iter** |

**训练配置：**

| 参数 | 值 |
|------|-----|
| 配置 | FastGen 原生 `config_dmd2.py`（VSD + GAN） |
| GPU | 4× RTX 5090 32GB（FSDP） |
| 优化器 | 标准 AdamW（fp32），lr=1e-5 |
| 批大小 | 4（1/GPU，无梯度累积） |
| 迭代数 | 2000 |
| 训练时间 | ~9 小时 |
| 每步时间 | ~16s |
...
**Loss 收敛：** 总损失从 1.38 降至 ~1.10（前 200 iter 快速下降），之后稳定在 1.04-1.17 范围。GAN 判别器损失：1.39 → 1.25。VSD 损失在 0.21-0.35 间波动。

### 5.5.4 DMD2 推理结果 — 视觉对比

所有视频参数：**4 步学生采样，seed=42，guidance_scale=5.0**
提示词：*"A cat walking on the grass in the sunshine"*

**Teacher 基线（50 步，170.88s）vs DMD2 Student（4 步，~6.7s）= 25.5× 加速**

| 模型 | 帧截图 (t=40) |
|:-----|:---:|
| **Teacher（50 步，170.88s）** | ![Teacher](results/frames/teacher_50step.jpg) |
| **DMD2 400 iter（4 步，6.73s）** | ![DMD2 400](results/frames/dmd2_400iter.jpg) |
| **DMD2 800 iter（4 步，6.81s）** | ![DMD2 800](results/frames/dmd2_800iter.jpg) |
| **DMD2 1200 iter（4 步，6.69s）** | ![DMD2 1200](results/frames/dmd2_1200iter.jpg) |
| **DMD2 1600 iter（4 步，6.67s）** | ![DMD2 1600](results/frames/dmd2_1600iter.jpg) |
| **DMD2 2000 iter（4 步，6.64s）** | ![DMD2 2000](results/frames/dmd2_2000iter.jpg) |
...
| DMD2 Student (4 步) | 4 | ~6.7s | **25.5×** |
| CausVid (预训练 DMD) | 3 | 28.5s | 6.0× |
| rCM (预训练) | 4 | 37.6s | 4.5× |

### 5.5.5 分析：为什么 400 iter 看起来比 2000 iter 好

一个意外的发现：DMD2 在 400 次迭代时的视觉质量似乎高于 2000 次迭代。这是**小 batch size 下 GAN 训练不稳定**的典型表现：

1. **Batch size 差距过大：** 我们使用 `batch=4`，FastGen 官方为 `batch=64`（16 倍差距）。GAN 判别器在极小 batch 下梯度估计噪声极大，导致 generator-discriminator 平衡容易被打破。

2. **判别器过强 (Discriminator Overpowering)：** 随着训练推进，判别器损失从 1.39 降到 1.25——判别器越来越强，迫使生成器产生"安全"但细节缺失的输出以最小化对抗损失。

3. **VSD 损失不稳定：** `vsd_loss` 在 0.21-0.35 之间大幅波动，未能单调下降，反映出 score matching 在小 batch 下的不稳定性。

4. **400 iter 处于"甜蜜点"：** 在 ~400 iter 时，生成器仍保留较多预训练质量，同时已开始学到蒸馏加速信号。进一步训练中 GAN 对抗的不稳定性逐渐累积，反而损害了质量。

**建议修复方案：** 通过梯度累积增加 batch size（`batch_global=16+`），降低 GAN 损失权重（0.03 → 0.01），或延长训练至 6000 iter（FastGen 官方配置）。

### 5.5.6 Phase 0.5 总结

| 方法 | 迭代 | GPU | 步数 | 单视频耗时 | 质量 | 状态 |
|------|------|-----|------|-----------|------|------|
| ECT | 1000 | 1 | 4 | — | 完全模糊 | ❌ |
| CD | 1000 | 1 | 4 | — | 仅有轮廓 | ⚠️ |
| CD | 5000 | 2 FSDP | 4 | — | 质量退化（curriculum 问题） | ❌ |
| **DMD2** | **2000** | **4 FSDP** | **4** | **6.7s** | **收敛，25.5× 加速** | **✅** |

**关键结论：**

1. **ECT 和 CD 是图像专用方法**，FastGen 未对视频验证。我们的实验证实它们无法生成可用的视频输出。

2. **DMD2 是我们硬件上唯一可行的视频蒸馏方法**。它最少需要 4× 32GB GPU + FSDP（8-bit AdamW 与 FSDP2 DTensor 不兼容）。

3. **DMD2 实现 25.5× 推理加速**（6.7s vs 170.88s），远超 Phase 0 预训练模型的结果（CausVid 6.4×，rCM 4.9×）。

4. **质量可通过更大 batch size、更多迭代（6000 iter）或 GAN 损失权重调优进一步提升**。
...
| DMD2（双卡 FSDP） | + FakeScore + 判别器 | **30.41 GB** | 否 (OOM) |
| **DMD2（4 卡 FSDP）** | + FakeScore + 判别器 | **22-27 GB** | **是 ✅** |
| f-distill | + FakeScore + 判别器 | **>31 GB** | 未测试（类似 DMD2） |

**根本原因：** FSDP 在前向传播时全收集完整参数。文本编码器（UMT5-XXL ~10GB）在每个 GPU 上复制。

**Phase 0.5 更新：** DMD2 已在 4 卡 FSDP 上成功训练（22-27 GB/卡）。关键发现：8-bit AdamW（bitsandbytes）与 FSDP2 DTensor 不兼容，必须使用标准 fp32 AdamW，因此最少需要 4 卡。
...
3. **显存是首要限制，但可解决：** DMD2 需要 4× 32GB GPU + FSDP（22-27 GB/卡）。8-bit AdamW 与 FSDP2 DTensor 不兼容。双卡 FSDP 在 30.41 GB/卡时 OOM。CPU 卸载可用但速度不实际（~1200s/iter vs 16s/iter）。

4. **ECT/CD 是图像专用方法——不适用于视频：** ECT（1000 iter）和 CD（1000-5000 iter）均无法生成可用的视频输出。FastGen 有意将 ECT/CD 排除在 WanT2V 配置之外。仅分布匹配方法（DMD2、f-distill、LADD、CausVid）被验证适用于视频。

5. **DMD2 从零训练可行但对 batch size 敏感：** 2000 iter DMD2 在 4 卡 FSDP 上稳定收敛（loss 1.38→1.10），实现 25.5× 推理加速。但小 batch size（4 vs 官方 64）导致 GAN 训练不稳定——质量在 ~400 iter 达到峰值后逐渐退化。

6. **RTX 5090 存在兼容性缺口：** Flash attention 库和自定义 CUDA 扩展不支持 Blackwell 架构（SM 12.0）。纯 PyTorch 回退可用但可能牺牲性能。
...
| 优先级 | 方法 | 理由 |
|--------|------|------|
| **首选** | DMD2（FastGen 原生） | Phase 0.5 成功训练；25.5× 加速；FastGen 官方方法（VBench 83.24） |
| **次选** | CausVid（DMD 预训练） | Phase 0 中 6.4× 加速；成熟的预训练检查点 |
| **备选** | rCM（一致性模型） | 4.9× 加速；NVlabs 有 Wan2.1 预训练检查点 |
| ~~已弃用~~ | ~~ECT/CD~~ | ~~不适用于视频——图像专用方法，Phase 0.5 已证实~~ |

### 7.3 Phase 1 硬件推荐

| 场景 | 模型 | 方法 | 最低 GPU 配置 | 推荐 GPU 配置 |
|------|------|------|-------------|-------------|
| **1.3B + DMD2** | Wan2.1-1.3B | DMD2 | **4x RTX 5090 32GB** | 8x RTX 5090 32GB |
| 14B + DMD2 | Wan2.1-14B | DMD2 | 8x A100 80GB | 8x H100 80GB |
...
| **DMD2（2000 iter, 4 卡 FSDP）** | 自训练 (Phase 0.5) | `/data/chenqingzhan/fastgen_output/fastgen/wan_dmd2/dmd2_4gpu_fsdp/checkpoints/` |
| DMD2 合并格式（400-2000 iter） | 从 FSDP 合并 | `/data/chenqingzhan/fastgen_output/dmd2_consolidated/dmd2_{400..2000}iter.pth` |
