...
# Usage: bash run_ect_single_gpu.sh [data_dir_or_wds_tag]
#
...
CONFIG_PATH="${CONFIG_PATH:-fastgen/configs/experiments/WanT2V/config_cm_ct.py}"
DATA_DIR="${1:-/data/datasets/OpenVid-1M/webdataset}"
RUN_NAME="${RUN_NAME:-ect_wan1.3b_single_gpu}"
LOG_PATH="${LOG_PATH:-$FASTGEN_OUTPUT_ROOT/ect_train.log}"
...
if [ ! -d "$FASTGEN_DIR" ]; then
    echo "[ECT] FASTGEN_DIR not found: $FASTGEN_DIR"
    exit 1
fi

if [ ! -e "$MODEL_PATH" ]; then
    echo "[ECT] MODEL_PATH not found: $MODEL_PATH"
    exit 1
fi
