#!/bin/bash
# run_selfforcing_fsdp.sh — Self-Forcing on Wan2.1-1.3B with FSDP
#
# Self-Forcing: Causal DMD variant (4 networks: CausalStudent+Teacher+FakeScore+Disc)
# Requires pretrained causal student (ode_init.pt from gdhe17/Self-Forcing)
#
# Usage:
#   bash run_selfforcing_fsdp.sh           # Full 6000-iter
#   SMOKE=1 bash run_selfforcing_fsdp.sh   # 10-iter smoke test
...
if [ "$SMOKE" = "1" ]; then
    ...
    RUN_NAME="selfforcing_smoke"
    echo "[Self-Forcing] === SMOKE TEST ==="
else
    MAX_ITER=6000; SAVE_ITER=1000; VAL_ITER=1000; LOG_ITER=50
    RUN_NAME="selfforcing_6000iter"
    echo "[Self-Forcing] === FULL TRAINING (6000 iter) ==="
fi

echo "[Self-Forcing] GPUs: ${CUDA_VISIBLE_DEVICES} (${NUM_GPUS} GPUs)"
...
echo "[Self-Forcing] Complete! Checkpoints: ${FASTGEN_OUTPUT_ROOT}/fastgen/wan_sf/${RUN_NAME}/checkpoints/"
