...
# Memory: 8-bit AdamW + 4-GPU FSDP sharding (~20 GB/GPU)
# Note: 2-GPU FSDP OOMs (30.41 GB/GPU); CPU offload incompatible with bitsandbytes
...
if [ "$SMOKE" = "1" ]; then
    ...
    VAL_ITER=500
    LOG_ITER=50
    RUN_NAME="dmd2_2gpu_fsdp"
    echo "[DMD2] === FULL TRAINING (2000 iter) ==="
fi

echo "[DMD2] GPUs: ${CUDA_VISIBLE_DEVICES}"
echo "[DMD2] 4-GPU FSDP + 8-bit AdamW"
echo "[DMD2] batch_global=4 (1 per GPU, no grad_accum), max_iter=${MAX_ITER}"
echo ""

torchrun --nproc_per_node=4 --standalone train_lowmem.py \
