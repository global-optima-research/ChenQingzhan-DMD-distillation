// DMD2 progress report deck builder V2.3 — 2026-07-27 sync to storyline @ commit 4408c88 (P8 two-column: advice moved to speaker notes)
// Spec: research/report_storyline_0728.md section 1 (as-built, synced verbatim) + research/presentation_wording_guide.md (main slides: no internal codenames).
// Numbers frozen at commit 0c28815; the only newly surfaced figures are teacher flow 2.05 (B5 frozen)
// and the panel-basis percentage "约 −13%~−19%" (ruling 40e7442). No other numeric changes.
// Build: node build_deck_v2.js  ->  DMD2_report_0728_V2.pptx   (QA_IMAGES=1 -> DMD2_report_0728_V2_qa.pptx)
...
const TEAL = "00796B"; // E2a line (GAN off, highest-quality arm) — orange is reserved for degradation/emphasis
...
function footer(s, suffix, src) {
  // page number derives from pageNo so inserting slides never desyncs the footers
...
// V2 global header system: kicker + 8-dot progress + short title + gray subtitle + navy anchor bar
...
// numeric cell with orange caveat marker "▲" (red-line surfaced inside the table)
function MARK(text, o, markOnly) {
...
/* ============================================================ S1 | P0 正式封面 */
{
  ...
  txt(s, "Wan2.1-T2V 少步蒸馏", { x: 0.9, y: 2.44, w: 11.5, h: 0.78, fontSize: 40, bold: true, color: NAVY });
  s.addText([
    { text: "25×", options: { color: ORANGE } },
    { text: " 加速下的质量退化诊断与归因", options: { color: NAVY } },
  ], { x: 0.9, y: 3.26, w: 11.5, h: 0.78, fontFace: FONT, fontSize: 40, bold: true, margin: 0 });
  txt(s, "以 DMD2 将 Wan2.1-T2V-1.3B 从 50 步教师蒸馏为 4 步学生(单条 165.24s → ≈6.6s),并对质量退化完成系统性的受控诊断与归因。", { x: 0.9, y: 4.42, w: 11.2, h: 0.66, fontSize: 14.5, color: INK, lineSpacingMultiple: 1.2 });
  txt(s, "汇报人", { x: 0.9, y: 5.5, w: 4, h: 0.2, fontSize: 10.5, color: GRAY });
  txt(s, "Chen Hing Chin (21205180)", { x: 0.9, y: 5.72, w: 8, h: 0.34, fontSize: 16, bold: true, color: INK });
  txt(s, "PVTT Task 3 · DMD 蒸馏与加速", { x: 0.9, y: 6.12, w: 8, h: 0.22, fontSize: 10.5, color: GRAY });
  footer(s, "", "出处:速度数字见 canonical 实验记录 W1 节(2026-07-26 直读 metrics.csv 复核)");
  s.addNotes("开场(10s):报题、自报姓名学号;点下一页进入同 prompt 对比视频。日期口径:项目里程碑为 2026-07-28 汇报;用户输入的 2026/6/28 判为笔误,已按里程碑用 2026/07/28——如确需 6/28,改本页首行文本即可。");
}
...
/* ============================================================ S2 | P1 开场 */
{
  const s = pres.addSlide(); pageNo++;
  header(s, {
    idx: 0, kicker: "开场|任务与问题",
    main: "25× 加速下,画面乍看接近",
    sub: "同一 prompt 对照:teacher 50 步 + CFG vs 4 步学生(无 CFG);本报告给出定量诊断与归因",
  });
  ...
  txt(s, "teacher:50 步 + CFG(≈100 NFE)\n单条 165.24s", { x: 1.31, y: 4.41, w: vw, h: 0.52, fontSize: 11, align: "center", color: GRAY });
  txt(s, "4 步学生(随机种子 1):无 CFG(4 NFE)\n单条 ≈6.6s", { x: 6.92, y: 4.41, w: vw, h: 0.52, fontSize: 11, align: "center", color: GRAY });
  s.addText([
  ...
  txt(s, "任务:将公开 Wan2.1-T2V-1.3B 的 50 步教师模型以 DMD2 蒸馏为 4 步学生;训练框架与全部超参承自 NVIDIA FastGen 公开配置。我们的工作:50→8→4 步数接力的训练日程,以及系统性的退化诊断与归因。", { x: 0.9, y: 5.85, w: 11.53, h: 0.62, fontSize: 13 });
  txt(s, "本报告回答的问题:加速 25 倍之后,生成质量损失了什么?损失来自哪个环节?", { x: 0.9, y: 6.5, w: 11.53, h: 0.4, fontSize: 14, bold: true, color: NAVY });
  footer(s, "", "出处:canonical 实验记录 W1 节(速度 2026-07-26 直读 metrics.csv 复核)");
...
/* ============================================================ S2 | P2 方法论 */
{
  ...
  header(s, {
    idx: 1, kicker: "① 量化协议",
    main: "主观选点在两条训练线上均被量化推翻",
    sub: "全部结论建立在四组件量化协议与 best-of-sweep 选点之上",
  });
  // left: hand-drawn line chart (deterministic coordinates), W7 aes by iter
  txt(s, "接力学生:美学质量(aesthetic,150 条标准提示词)随训练迭代单调下降 0.5768 → 0.5379", { x: 0.55, y: 1.42, w: 7.1, h: 0.28, fontSize: 12, bold: true });
  txt(s, "量化最优:第 500–1000 步", { x: 1.42, y: 1.74, w: 2.6, h: 0.24, fontSize: 10.5, bold: true, color: ORANGE });
  txt(s, "纵轴 0.535–0.582(非零起点)", { x: 4.75, y: 1.78, w: 2.2, h: 0.18, fontSize: 8, color: GRAY, align: "right" });
  const px = [1.35, 2.75, 4.15, 5.55, 6.95];
  const vals = [0.5768, 0.5592, 0.5433, 0.5477, 0.5379];
  const y0 = 4.78, yTop = 2.04, f = 2.74 / 0.047; // tightened range 0.535-0.582: curve fills the plot area
  const py = vals.map((v) => y0 - (v - 0.535) * f);
  // quantitative-optimum interval as a light-orange background band (iter 500-1000)
  ...
  txt(s, "训练迭代", { x: 1.35, y: 5.08, w: 5.6, h: 0.2, fontSize: 9.5, color: GRAY, align: "center" });
  ...
  // subjective pick @2500: orange hollow ring around the end point
  s.addShape(ELL, { x: 6.95 - 0.085, y: py[4] - 0.085, w: 0.17, h: 0.17, fill: { type: "none" }, line: { color: ORANGE, width: 1.75 } });
  txt(s, "主观选择:第 2500 步(下降段末端)", { x: 4.15, y: 3.5, w: 2.5, h: 0.42, fontSize: 10.5, color: INK });
  seg(s, 5.95, 3.94, 6.84, py[4] - 0.12, { color: "AAAAAA", width: 1 });
  // right: W4 counter-example
  txt(s, "反例:均匀时间步消融——协议的阳性对照", { x: 7.7, y: 1.42, w: 5.13, h: 0.28, fontSize: 12, bold: true });
  s.addTable([
  ...
  txt(s, "仅看一致性类指标,会将一个已坍缩为近静态输出的模型误判为最优。", { x: 7.7, y: 3.54, w: 5.13, h: 0.6, fontSize: 11.5, bold: true, color: NAVY });
  txt(s, "由此立联读规则:一致性类指标必须与动态度、多样性联读。", { x: 7.7, y: 4.22, w: 5.13, h: 0.4, fontSize: 9.5, color: GRAY });
  // bottom band
  s.addShape(RR, { x: 0.5, y: 5.62, w: 12.33, h: 1.08, rectRadius: 0.06, fill: { color: LIGHT }, line: { color: BORDER, width: 0.75 } });
  txt(s, "由此建立四组件量化协议:质量六维(150 条标准提示词)/ 动态度(40 条运动类提示词)/ 跨种子多样性(40 条 × 8 种子)/ 连续光流(RAFT)——细节见备页 B1。\n选点规则:每 500 迭代存档、全量扫描取优 + 最优存档点以 3 个随机种子复测。", { x: 0.78, y: 5.78, w: 11.8, h: 0.85, fontSize: 12, lineSpacingMultiple: 1.25 });
  footer(s, "", "出处:experiments/results/2026-07-14-e0-full-table-g1.md");
  s.addNotes("讲稿(60s):两条训练线的主观选点方向相反地出错——W7 主观选第 2500 步(实为下降段末端),W1 主观选第 1000 步(量化 @1500 在 5/6 质量维反超)→ 这是协议问题而非单次失误。W4(均匀 t_list)说明常用指标存在系统性盲区。伏笔(必讲):“质量在训练早期见顶后回落——这一现象的机制在“④ 判别器审计”一页回答。”红线:不主张 t_list 消融首创(TMD 已有);一致性指标必须与动态/多样性联读。");
}
...
/* ============================================================ S3 | P3 核心发现 */
{
  ...
  header(s, {
    idx: 2, kicker: "② 主退化轴",
    main: "少步蒸馏的主要退化轴是跨 seed 多样性",
    sub: "teacher 0.732 → 学生 0.59–0.64;动态度并未下降",
  });
  txt(s, "同一 prompt(“An astronaut walking slowly across a dusty red desert…”,与训练数据同域的提示词):仅随机种子不同;缩略图为第 40 帧,可点击播放", { x: 0.5, y: 1.32, w: 12.33, h: 0.2, fontSize: 9.5, color: GRAY });
  const cw = 1.314, ch = 0.758, gap = 0.028, x0 = 2.22;
  const tSeeds = [0, 1, 2, 3, 4, 5, 6, 7];
  const eSeeds = [0, 2, 3, 4, 6, 8, 9, 11];
  // row label column with punchline hooks
  txt(s, "teacher · 50 步", { x: 0.4, y: 1.58, w: 1.7, h: 0.22, fontSize: 10, bold: true });
  txt(s, "165.24s/条", { x: 0.4, y: 1.8, w: 1.7, h: 0.16, fontSize: 7.5, color: GRAY });
  txt(s, "8 个 seed,8 种构图", { x: 0.4, y: 2.0, w: 1.7, h: 0.36, fontSize: 9, bold: true, color: NAVY });
  txt(s, "学生(4 步直蒸)", { x: 0.4, y: 2.59, w: 1.7, h: 0.22, fontSize: 10, bold: true });
  txt(s, "≈6.6s/条", { x: 0.4, y: 2.81, w: 1.7, h: 0.16, fontSize: 7.5, color: GRAY });
  txt(s, "8 个 seed,趋同一种模板", { x: 0.4, y: 3.01, w: 1.7, h: 0.36, fontSize: 9, bold: true, color: ORANGE });
  tSeeds.forEach((sd, k) => {
    const x = x0 + k * (cw + gap);
  ...
  // bottom double panel (hand-drawn horizontal bars)
  ...
  txt(s, "动态度(40 条运动类提示词):未下降", { x: 0.5, y: 3.62, w: 5.95, h: 0.26, fontSize: 12, bold: true });
  ...
  txt(s, "0.75–1.0(全体学生)", { x: 4.84, y: 4.37, w: 1.5, h: 0.2, fontSize: 9.5, bold: true, align: "center" });
  txt(s, "未降反升 ↑", { x: 4.99, y: 5.0, w: 1.2, h: 0.2, fontSize: 9.5, bold: true, color: ORANGE, align: "center" });
  ...
  // panel B — diversity, zoomed 0.4-0.8 scale (declared), band 2.5x wider than the 0-1 rendering
  const posB = (v) => barX2 + ((v - 0.4) / 0.4) * barW;
  txt(s, "跨种子多样性(LPIPS):一致下降——主退化轴", { x: 6.88, y: 3.62, w: 5.95, h: 0.26, fontSize: 12, bold: true, color: ORANGE });
  ...
  txt(s, "全体学生(各自最优档)", { x: posB(0.59) - 0.65, y: 4.37, w: 1.8, h: 0.2, fontSize: 9.5, bold: true, color: ORANGE, align: "center" });
  ...
  txt(s, "横轴 0.4–0.8(非零起点)。相对 teacher 约 −13%~−19%(按本图数值);弱/强配方、直蒸/接力全部同向——本项目复现次数最多的发现", { x: 6.88, y: 5.5, w: 5.95, h: 0.4, fontSize: 9.5, color: GRAY });
  ...
  footer(s, "", "出处:E0 全表(07-14) · G2 定稿表(07-20)");
  s.addNotes("讲稿(75s):文献普遍担忧少步蒸馏损失动态;我们的量化结果是“动态未降、运动平滑 0.97+ 排除抖动伪动态”(与文献实验域不同,表述为“未复现”而非反驳)。真正一致下降的是多样性——弱配方、强配方、直蒸、接力全部同向,是本项目复现次数最多的发现。多样性定义一句(见页面底部)。视频墙:上排 teacher 8 seed 内容彼此明显不同,下排学生 8 seed 构图明显趋同(学生排为 seed 0/2/3/4/6/8/9/11,如实标注)。种子非连续备答(裁定 2026-07-27,版面不披露):个别种子生成崩坏,按可用种子顺位替换;定量多样性结论基于全量 8 种子协议,与演示素材无关(MANIFEST 在案)。过渡句(固定):“该退化在所有配置中出现——下面通过两组受控消融定位其来源。”百分比口径(裁定 2026-07-27):标题不带百分比,正文用面板口径准确值“约 −13%~−19%”。\n\n出处全名:experiments/results/2026-07-14-e0-full-table-g1.md、2026-07-20-g2-relay-vs-direct-final.md。");
}
...
/* ============================================================ S4 | P4 消融一 */
{
  const s = pres.addSlide(); pageNo++;
  header(s, {
    idx: 3, kicker: "③ 受控对照|接力 vs 直蒸",
    main: "接力无质量收益、多样性更低",
    sub: "匹配预算受控对照(G2);唯一稳定效应是更高的运动幅值",
  });
  ...
  s.addTable([
    ...
    [C("直蒸学生 @1000(主对照)", { fontSize: 10 }), RN("0.567"), MARK("0.717", { bold: true, underline: true }), MARK("0.750"), RN("0.635")],
    [C("接力学生 @500", { fontSize: 10 }), RN("0.577"), RN("0.694"), RN("0.825"), RN("0.598")],
    [C("接力学生 @1000", { fontSize: 10 }), RN("0.559"), RN("0.697"), BU("1.000", { align: "right", fontSize: 10.5 }), RN("0.613")],
    [C("直蒸学生 B @500(高学习率)", { fontSize: 10 }), RN("0.532"), RN("0.695"), RN("0.975"), RN("0.628")],
  ...
  txt(s, "协议:美学/成像 = 150 条标准提示词;动态度 = 40 条运动类提示词;多样性 = 40 条 × 8 种子(LPIPS);各列最优加粗下划线", { x: 0.5, y: 3.92, w: 7.35, h: 0.2, fontSize: 8.5, color: GRAY });
  s.addText([
    { text: "▲ ", options: { color: ORANGE, bold: true } },
    { text: "口径限定:0.717 高于 teacher 系锐度偏置,不作“超越”解读;0.750 低于其余学生(0.825–1.0),如实标注", options: { color: INK } },
  ], { x: 0.5, y: 4.15, w: 7.35, h: 0.26, fontFace: FONT, fontSize: 10.5, margin: 0 });
  // videos bottom-left
  txt(s, "同 prompt·同 seed 对照(cyclist):运动幅值差异直观呈现", { x: 0.6, y: 4.52, w: 6.1, h: 0.2, fontSize: 8.5, color: GRAY });
  video(s, "p4/p4_w7_cyclist.mp4", 0.6, 4.74, 2.78, 1.604);
  video(s, "p4/p4_e1a_cyclist.mp4", 3.72, 4.74, 2.78, 1.604);
  txt(s, "接力学生(4 步)", { x: 0.6, y: 6.4, w: 2.78, h: 0.2, fontSize: 9, align: "center", color: GRAY });
  txt(s, "直蒸学生(4 步)", { x: 3.72, y: 6.4, w: 2.78, h: 0.2, fontSize: 9, align: "center", color: GRAY });
  // right-top design box: fork schematic + invariants + pre-registration badges
  s.addShape(RR, { x: 8.1, y: 1.42, w: 4.73, h: 2.28, rectRadius: 0.06, fill: { color: PANEL }, line: { color: NAVY, width: 1 } });
  txt(s, "受控设计(预注册)", { x: 8.3, y: 1.54, w: 4.35, h: 0.26, fontSize: 12, bold: true, color: NAVY });
  chip(s, 8.3, 2.06, 1.02, 0.32, "teacher 50 步");
  // relay branch
  seg(s, 9.36, 2.14, 9.66, 2.01, { color: GRAY, width: 1.25, endArrowType: "triangle" });
  chip(s, 9.68, 1.86, 0.94, 0.3, "8 步中间模型");
  seg(s, 10.64, 2.01, 10.98, 2.01, { color: GRAY, width: 1.25, endArrowType: "triangle" });
  txt(s, "全重置 ↺", { x: 10.44, y: 2.19, w: 0.75, h: 0.15, fontSize: 7.5, bold: true, color: ORANGE, align: "center" });
  chip(s, 11.0, 1.86, 0.86, 0.3, "4 步(接力)");
  txt(s, "接力路径\n2×2500 迭代", { x: 11.92, y: 1.83, w: 0.79, h: 0.36, fontSize: 7.5, color: GRAY });
  // direct branch
  seg(s, 9.36, 2.3, 10.58, 2.55, { color: GRAY, width: 1.25, endArrowType: "triangle" });
  chip(s, 10.6, 2.42, 1.14, 0.3, "4 步(直蒸 ×2)");
  txt(s, "直蒸路径\n各 5000 迭代", { x: 11.8, y: 2.42, w: 0.9, h: 0.36, fontSize: 7.5, color: GRAY });
  txt(s, "不变量:总预算 · 数据 · 时间步设置 · 训练配方 · 评估协议;直蒸设高/低学习率两组,防“基线未调优”", { x: 8.3, y: 2.88, w: 4.35, h: 0.34, fontSize: 9, lineSpacingMultiple: 1.1 });
  s.addShape(RR, { x: 8.3, y: 3.26, w: 2.08, h: 0.32, rectRadius: 0.05, fill: { color: "FFFFFF" }, line: { color: NAVY, width: 1 } });
  txt(s, "预注册预期:两者相当", { x: 8.3, y: 3.26, w: 2.08, h: 0.32, fontSize: 9.5, color: NAVY, align: "center", valign: "middle" });
  s.addShape(RR, { x: 10.53, y: 3.26, w: 2.08, h: 0.32, rectRadius: 0.05, fill: { color: NAVY }, line: { color: NAVY, width: 1 } });
  txt(s, "实测:直蒸略优", { x: 10.53, y: 3.26, w: 2.08, h: 0.32, fontSize: 9.5, bold: true, color: "FFFFFF", align: "center", valign: "middle" });
  // right-bottom flow bars with teacher reference line
  txt(s, "运动幅值(RAFT 光流中位,dm40,4-seed 均值)", { x: 8.1, y: 3.92, w: 4.73, h: 0.24, fontSize: 11, bold: true });
  txt(s, "接力学生", { x: 8.1, y: 4.3, w: 1.15, h: 0.3, fontSize: 9.5, align: "right" });
  txt(s, "直蒸学生", { x: 8.1, y: 4.8, w: 1.15, h: 0.3, fontSize: 9.5, align: "right" });
  ...
  txt(s, "4/4 种子方向一致,约 1.9×;虚线为 teacher 参考(4 种子均值 2.05):接力高于(4/4)、直蒸不高于(3/4)teacher。评价取决于以贴近 teacher 分布还是以更大动态为准则——本文不选边(全表见备页 B5)", { x: 8.1, y: 5.48, w: 4.73, h: 0.85, fontSize: 9.5, color: GRAY });
  footer(s, "", "出处:G2 定稿表(07-20) · ch2 发现 1/5 · flow 多 seed(07-23)");
  s.addNotes("讲稿(90s):接力是我们的原始设计,因此这项对照以预注册方式进行以避免偏向;结论如实:质量判平或直蒸略优(imaging E1a 0.717 为学生最高;aes n=3 带重叠判平)、多样性直蒸双臂一致更高(0.635/0.628 vs 0.598/0.613);运动幅值是接力唯一稳定的实测差异,其来源在下页消融中回答。红线:E1a 成像高于 teacher 只按锐度/静态偏置口径讲;E1a 动态度 0.75 偏低须如实提;单 seed 百分比禁引;不宣称直蒸路线首创。\n\n备问卡 #1「负结果为什么值得报告?」——两个负结果均为受控排除的进展:接力对照是该基座上首个受控的 relay-vs-direct 比较(GPD/CoDMD/FastWan 均未做),并给出接力唯一的真实效应(运动幅值,4/4 seed)。\n\n出处全名:2026-07-20-g2-relay-vs-direct-final.md、research/thesis_ch2_draft.md 发现 1/5、2026-07-23-flow-multiseed-e1b946-e2a-eval.md。运动幅值 teacher 参考线 2.05 = 备页 B5 的 4-seed 均值(冻结值;设计裁项 B2 采纳)。");
}
...
/* ============================================================ S5 | P5 消融二 */
{
  const s = pres.addSlide(); pageNo++;
  header(s, {
    idx: 4, kicker: "④ 判别器审计|单变量消融",
    main: "GAN 分支驱动“质量随训练回落”与运动幅值",
    sub: "同一初始化(同一 8 步中间模型)、同一配方,仅 GAN 设置不同;(t,ε) 配对约定未检测到影响",
  });
  txt(s, "美学质量(aesthetic)随训练迭代:三组对比", { x: 0.55, y: 1.45, w: 7.3, h: 0.27, fontSize: 11.5, bold: true });
  ...
  txt(s, "纵轴 0.53–0.62(非零起点)", { x: 1.42, y: 1.98, w: 1.9, h: 0.18, fontSize: 8, color: GRAY });
  ...
  txt(s, "≈0.577(两组同起点)", { x: 1.45, y: 3.72, w: 1.8, h: 0.18, fontSize: 8.5, color: GRAY });
  txt(s, "0.6109(峰)@2000", { x: 4.28, y: e2a[3] - 0.28, w: 1.5, h: 0.18, fontSize: 8.5, color: TEAL });
  txt(s, "GAN 关闭\n终值 0.6074", { x: 6.15, y: e2a[4] - 0.2, w: 1.7, h: 0.42, fontSize: 10, bold: true, color: TEAL });
  txt(s, "GAN 开启 · 独立 (t,ε)\n终值 0.5487", { x: 6.15, y: e2b[4] - 0.34, w: 1.72, h: 0.55, fontSize: 9, color: NAVY });
  txt(s, "GAN 开启 · 共享 (t,ε)\n终值 0.5379", { x: 6.15, y: w7[4] - 0.02, w: 1.72, h: 0.55, fontSize: 9, color: INK });
  // right column: three conclusion boxes (heights fitted to content)
  ...
  box(1.5, 1.2, "① 质量“早峰后滑”← GAN 分支(候选机制)", "关闭 GAN(E2a):质量随训练一路向好,五档同向;开启 GAN(E2b/W7):随训练回落——现象在单变量对照中消失并反向。冠军档 n=3 逐 seed 配对 3/3 同向。", ORANGE);
  box(2.95, 1.2, "② 运动幅值主要由 GAN 分支维持", "关闭 GAN:回落至 teacher 水平(2.1–2.7);开启 GAN:自低点重建至 4.71。在匹配配方内与接力初始化存在交互(直蒸学生开启 GAN 仍低)。");
  box(4.4, 1.2, "③ (t,ε) 配对约定:未检测到效应", "real/fake 共享 (t,ε) 为上游默认设计,据我们所知此前无受控检验;五个对照档质量差均 ≤0.011,各维走势同构。");
  s.addShape(RR, { x: 0.5, y: 6.42, w: 12.33, h: 0.5, rectRadius: 0.05, fill: { color: LIGHT }, line: { color: BORDER, width: 0.75 } });
  txt(s, "三组的跨种子多样性均落在 0.586–0.613——对 GAN 开关与配对方式均不敏感(下一页收束)。", { x: 0.78, y: 6.52, w: 11.8, h: 0.3, fontSize: 11.5, bold: true });
  footer(s, "", "出处:E2a 全表(07-24) · E2b 三臂全表(07-25)");
  s.addNotes("讲稿(105s):先一句交代消融设计的干净性——在接力臂 W7 配方上每次只改一个字段(E2a 仅 gan_loss_weight_gen 0.03→0;E2b 仅 gan_use_same_t_noise True→False),配置逐值核对。中心图讲 20 秒(同一初始化、同一配方,仅 GAN 设置不同);三个结论各 15 秒;强调第③条的价值:被广泛复制的默认设计首次得到受控检验。红线:“候选机制”措辞(单权重点 0.03、单血统);禁“GAN 必然抬高运动”(直蒸臂 E1a 开启 GAN 仍低);E2a 美学 0.613 高于 teacher 0.590 不作对比表述;配对结论说“未检测到效应”而非“无差异”。图例色:E2a 青绿实线、E2b 深蓝实线、W7 浅蓝虚线(橙色全 deck 保留给主退化轴/警示)。\n\n备问卡 #3「关闭 GAN 后画质更高,为何不建议关闭?」——单权重点(0.03)、单血统(接力)结论;且关闭后运动幅值回落至 teacher 水平——是取舍不是免费改进;本工作给出的是归因而非配方建议。\n\n出处全名:2026-07-24-e2a-fulltable-ch3.md、2026-07-25-e2b-fulltable-ch3-threearm.md。");
}
...
/* ============================================================ S6 | P6 归因收束 */
{
  ...
  header(s, {
    idx: 5, kicker: "⑤ 归因收束",
    main: "逐部件受控排除后,多样性坍缩仍然存在",
    sub: "在受测部件内,归因于分布匹配蒸馏本身;未测部件如实列出",
  });
  const comp = (y, name, ev) => {
    ...
    txt(s, "已受控排除", { x: 3.95, y: y + 0.14, w: 1.4, h: 0.22, fontSize: 8.5, color: INK, align: "center" });
  ...
  comp(1.7, "步数接力(50→8→4)", "证据:③ 受控对照——接力组多样性反而更低(0.598/0.613 vs 直蒸 0.628/0.635)");
  comp(3.0, "GAN 判别器分支", "证据:④ 判别器审计——开关不改变坍缩量级(三组 0.586–0.613)");
  comp(4.3, "(t,ε) 配对约定", "证据:④ 判别器审计——五个对照档均未检测到效应");
  txt(s, "受控排除后仍然存在:跨 seed 多样性坍缩", { x: 6.4, y: 1.7, w: 6.4, h: 0.3, fontSize: 12.5, bold: true });
  const bx = 7.75, bw = 4.2; // zoomed scale 0.4-0.8 (declared)
  ...
  txt(s, "跨种子多样性(LPIPS,40 条 × 8 种子;全体学生 = 各组最优存档点与弱配方参考;横轴 0.4–0.8,非零起点)", { x: 6.4, y: 4.22, w: 6.4, h: 0.22, fontSize: 8.5, color: GRAY });
  ...
  footer(s, "", "出处:2026-07-25-e2b-fulltable-ch3-threearm.md(裁读 4)");
...
/* ============================================================ S7 | P7 VBench */
{
  ...
  header(s, {
    idx: 6, kicker: "⑥ 外部基准",
    main: "标准基准上不存在支配模型",
    sub: "full VBench 12/16 维(946 prompts × 5 seeds):不同模型在不同维度领先;多样性上界仍属 teacher",
  });
  ...
  s.addTable([
    [HC("维度", { align: "left" }), HC("直蒸学生 @1000 · 主对照"), HC("接力学生 @1000"), HC("直蒸学生 B @500"), HC("关闭 GAN @2000 · 消融组 · 仅单档")],
  ...
  txt(s, "每行最优 = 加粗下划线 + 浅橙底。缺失 4 维均为检测器依赖维(GRiT/detectron2),官方 Semantic 与 Total 不可合成,如实声明;temporal_flickering 官方协议为专属子集(25 样本/prompt、static_filter 预筛),本表 5 样本未筛,不与官方数字直比。", { x: 0.45, y: 6.14, w: 8.55, h: 0.44, fontSize: 9.5, color: GRAY });
  // right reading column
  txt(s, "读法(各有所长)", { x: 9.25, y: 1.42, w: 3.6, h: 0.28, fontSize: 12, bold: true, color: NAVY });
  ...
  rb(1.8, "直蒸(主对照)", "一致性 / 平滑 / 闪烁类领先");
  rb(2.7, "接力", "动态度 0.911 与动作语义类(0.794 / 0.292)领先");
  rb(3.6, "关闭 GAN(消融组)", "静态画质最高(0.6482 / 0.6924),动态 0.80 未塌——与消融结论跨域同向");
  rb(4.74, "teacher", "多样性 0.732(自建协议度量)仍为上界——该轴不在 VBench 维度内,正是自建协议的原因(备页 B4)");
  footer(s, "", "出处:experiments/results/2026-07-26-e2a-vb946-fourth-row.md");
  s.addNotes("讲稿(60s):四模型各有所长——E1a 赢一致性/平滑/闪烁类,W7 赢动态度与动作语义类,E1b 居中,E2a 静态画质最高且动态不塌(与 q150 域消融结论跨域同向)。行最优的浅橙底热区直观呈现“四片领先区”。多样性不在 VBench 维度内(自建协议的原因)。红线:最终模型选择遵循 G2 预注册结果(E1a@1000),非事后挑选;合成分与文献数字巧合仅在备页 B4/备问卡处理,不上正片;两域动态度数字(q150 0.567 / vb946 0.800)不混引。\n\n备问卡 #2「与 CoDMD 的 84.46 相比如何?」——协议不可比(我们 12/16 维、5 样本 flickering、无 GRiT 维),该数字仅作文献坐标,不做 SOTA 对比;我方 W7 的 Quality Score 84.47 与其为数字巧合(7 维质量合成 vs 16 维总分),不并列。\n备问卡 #6「主对照 E1a 的合成分为何最低(82.80)?」——合成分权重结构:动态度以 0.5 权重计入,E1a 动态偏低(0.5806)拖低总分,其领先的一致性/闪烁维归一化后区分度小;且合成分不含多样性项,无法反映主退化轴。模型选择依据是预注册协议,合成分仅作参考。\n备问卡 #7「E2a 合成分最高(85.50),为何不改选它?」——①预注册纪律:最终模型在 E2a 存在之前已按协议确定,事后更换即选择性报告;②E2a 多样性为全场最低带(0.586–0.604)、动态回落至 teacher 水平——合成分恰好测不到这两点;③单次训练、单权重点。");
}
...
/* ============================================================ S8 | P8 结论 */
{
  ...
  col(0.5, "✓", "主要结论", [
    ...
    "工程:25× 加速;该基座上首个受控的接力-直蒸对照(据我们所知)",
  ]);
  col(6.93, "⚠", "局限与后续", [
    "无人工评测;标准基准缺 4 维",
    "R1 正则臂受 32G 显存限制未测(校准值留档,80G 级设备可复现)",
    "各配置均为单次训练(同类配置方向互证);接力中间模型的选点早于选点规则确立",
    "后续:人评 · 跨血统消融 · 80G 复现 · 多样性坍缩的干预研究",
  ]);
  // closing number band — the three figures to remember
  s.addShape(RR, { x: 0.5, y: 6.08, w: 12.33, h: 0.58, rectRadius: 0.06, fill: { color: NAVY }, line: { type: "none" } });
  s.addText([
    { text: "加速 ", options: { fontSize: 11, color: "FFFFFF" } },
    { text: "165.24s → ≈6.6s(≈25×)", options: { fontSize: 15, bold: true, color: "FFFFFF" } },
    { text: "    ｜    ", options: { fontSize: 12, color: NAVY_SOFT } },
    { text: "多样性 ", options: { fontSize: 11, color: "FFFFFF" } },
    { text: "0.732 → 0.59–0.64", options: { fontSize: 15, bold: true, color: "FFFFFF" } },
    { text: "    ｜    ", options: { fontSize: 12, color: NAVY_SOFT } },
    { text: "接力 · GAN · (t,ε) ", options: { fontSize: 11, color: "FFFFFF" } },
    { text: "三部件已受控排除", options: { fontSize: 15, bold: true, color: "FFFFFF" } },
  ], { x: 0.7, y: 6.08, w: 11.93, h: 0.58, fontFace: FONT, align: "center", valign: "middle", margin: 0 });
  txt(s, "谢谢!欢迎指正——论文将于 2026-07-31 提交。", { x: 0.5, y: 6.78, w: 12.33, h: 0.3, fontSize: 12, bold: true, color: NAVY, align: "center" });
  footer(s, "", "出处:T3 裁定 §4.1 · E2b 三臂全表(07-25) · 台账 #11–#13");
  s.addNotes("讲稿(60s):两栏各 ~22 秒,收尾指底部数字带(加速 25×、多样性 0.732→0.59–0.64、三部件受控排除),节奏更从容。红线:“首个”带“据我们所知”限定;实践建议仅口答,建议③口答必带 caveat(单权重点、单一训练路径)。\n\n实践建议三条(不上版面,被问“对后来者的建议”时口答;final report 结论章保留完整版):①每 500 迭代存档、全量扫描选点(质量峰值通常在训练早期);②监控跨种子多样性与连续光流(常规指标与合成分不报警);③GAN 权重是“静态画质 ↔ 运动幅值”的取舍参数,(t,ε) 配对无需调整——口答必带 caveat(单权重点、单一训练路径,外推需谨慎)。\n\n备问卡 #4「为什么没有人工评测?」——时间窗内优先完成受控消融;人评已列入局限与后续工作(计划 T2VHE 式对 teacher 评测)。\n备问卡 #5「R1 正则臂为什么未测?」——确定性 OOM(32G 单卡,崩溃点为 R1 的第二次判别器前向),非配方失败;校准值与配置已留档,80G 级设备可直接复现。\n\n出处全名:research/T3_novelty_adjudication.md §4.1、2026-07-25-e2b-fulltable-ch3-threearm.md、experiments/acceptance-log.md #11–#13。");
}
...
/* ============================================================ S9 | B1 协议 */
{
  ...
  footer(s, "(备页 B1)", "出处:research/thesis_ch1_draft.md §1.7");
...
/* ============================================================ S10 | B2 E5 */
{
  ...
  footer(s, "(备页 B2)", "出处:research/E5_probe_results.md(修正口径,权威)");
...
/* ============================================================ S11 | B3 G2 设计细节 */
{
  ...
  footer(s, "(备页 B3)", "出处:research/thesis_ch2_draft.md §2.1");
  s.addNotes("被问 G2 设计细节(为什么可信、防了哪些质疑)时展开。路径示意图见正片“③ 受控对照”页受控设计框(teacher 分叉 → 接力 50→8→4 全重置 / 直蒸 50→4)。");
}
...
/* ============================================================ S12 | B4 QS */
{
  ...
  txt(s, "警示:W7 的 84.47 与 CoDMD 文献值 84.46 为数字巧合——前者为 7 维 Quality Score、后者为 16 维 Total,量纲不同,禁止任何并列或比较表述。", { x: 0.85, y: 6.0, w: 8.1, h: 0.6, fontSize: 10.5, bold: true, color: DARKRED });
  ...
  footer(s, "(备页 B4)", "出处:experiments/results/2026-07-26-e2a-vb946-fourth-row.md(QS 节)");
...
/* ============================================================ S13 | B5 flow 多 seed */
{
  ...
  footer(s, "(备页 B5)", "出处:experiments/results/2026-07-23-flow-multiseed-e1b946-e2a-eval.md §1/§4");
  s.addNotes("被问运动幅值证据强度时展开;正片“③ 受控对照”页只报 4-seed 均值与方向(其虚线参考 2.05 即本表 teacher 4-seed 均值)。");
}
...
/* ============================================================ S14 | B6 上游关系 */
{
  ...
  s.addShape(RR, { x: 0.7, y: 1.5, w: 12.0, h: 2.75, rectRadius: 0.05, fill: { color: PANEL }, line: { color: BORDER, width: 0.75 } });
  txt(s, "我们的全部训练基于 NVIDIA FastGen(NVlabs/FastGen,Apache-2.0),复用其原生 DMD2 实现与官方 Wan2.1-T2V-1.3B 配置——包括 teacher CFG=5、生成端 GAN 权重 0.03、real/fake 共享 timestep 与噪声(gan_use_same_t_noise=True 为官方 Wan 配置出厂值)、teacher 第 15/22/29 层特征上的 multiscale MLP 判别器、student_update_freq=5 的 two-time-scale 更新,以及 4-step t_list=[0.999, 0.937, 0.833, 0.624, 0.0]。\n\n在此之上,我们的配方贡献限于训练日程层:官方仓库仅提供从 50-step teacher 一次蒸到 4-step/2-step 的单阶段配置,我们改为 50→8→4 的 step-count relay(步数接力),新增 8-step 中间 student 阶段,并规定 4-step 阶段仅继承 8-step 最优 checkpoint 的生成器权重、优化器/fake score/判别器全部重新初始化;数据侧选用 OpenVid-1M(上游不绑定数据集)。", { x: 1.0, y: 1.72, w: 11.4, h: 2.4, fontSize: 11, lineSpacingMultiple: 1.25 });
  txt(s, "此外的贡献:本报告的整套受控审计(接力对照、三臂判别器消融)与可移植的退化评估协议。判别器表述统一为:冻结 teacher backbone 第 15/22/29 层特征 + 可训练 multiscale MLP 头(2026-07-06 代码核实)。", { x: 0.7, y: 4.5, w: 12.0, h: 0.7, fontSize: 10.5, color: INK });
  footer(s, "(备页 B6)", "出处:research/T3_novelty_adjudication.md §6.2");
  s.addNotes("被问“你们与 FastGen 什么关系 / 哪些是你们做的”时直接出示本页并照读。不称“改进了 FastGen”;不把单阶段超参说成我方设计。");
}

const OUT = QA_IMAGES ? "DMD2_report_0728_V2_qa.pptx" : "DMD2_report_0728_V2.pptx";
